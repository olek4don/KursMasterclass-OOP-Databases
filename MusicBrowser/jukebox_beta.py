...
class Scrollbox(tkinter.Listbox):
    ...
    def grid(self, row, column, sticky='nsw', rowspan=1, columnspan=1, **kwargs):
        # tkinter.Listbox.grid(self, row=row, column=column, sticky=sticky, rowspan=rowspan, columnspan=columnspan, **kwargs)     # Python 2
...
class DataListBox(Scrollbox):
    ...
    def clear(self):
        self.delete(0, tkinter.END)     # tkinter.END constant represents the last value in the list
    ...
    def requery(self, link_value=None):
        ...
        if self.linked_box:
            self.linked_box.clear()     # so that way when we select a new artist the songs for the previous album aren't still displayed
...
if __name__ == '__main__':
    ...
    # testList = range(0, 100)          #   JUST FOR
    # albumLV.set(tuple(testList))      #   TESTING
    mainWindow.mainloop()
